...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        bestChoice = None
        minDist = float('inf')
        dist_iter = iter(livingGhostPositionDistributions)

        # Initial loop
        newDist = next(dist_iter, None)
        while newDist is not None:
            newPos = newDist.argMax()
            dist = self.distancer.getDistance(pacmanPosition, newPos)
        ...
        legal_iter = iter(legal)

        # Second loop
        action = next(legal_iter, None)
        while action is not None:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            dist = self.distancer.getDistance(successorPosition, bestChoice)

            if dist <= newMinDist:
                newMinDist = dist
                newBestChoice = action

            action = next(legal_iter, None)

        return newBestChoice
