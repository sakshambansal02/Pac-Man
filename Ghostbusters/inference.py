...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for var in variables:
        ...
        if var == PAC or var == GHOST0 or var == GHOST1:
            x = 0
            while x < X_RANGE:
                y = 0
                while y < Y_RANGE:
            ...
            i = 0
            while i < MAX_NOISE + X_RANGE + Y_RANGE - 1:
                variableDomainsDict[var].append(i)
                i += 1
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        variable = eliminationOrder[0]  # Choose the first variable for the initial execution

        while True:
            allFactors, newFactor = joinFactorsByVariable(allFactors, variable)

            if newFactor.unconditionedVariables() != {variable}:
                newFactor = eliminate(newFactor, variable)
                allFactors.append(newFactor)

            if variable == eliminationOrder[-1]:  # Check if the last variable is processed
                break

            # Move to the next variable for the next iteration
            variable = eliminationOrder[eliminationOrder.index(variable) + 1]

        result = normalize(joinFactors(allFactors))
        return result
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        if total_sum != 0:
            updated_probabilities = {key: value / total_sum for key, value in self.items()}
            self.update(updated_probabilities)
    ...
    def sample(self):
        ...
        key_iter = iter(self.items())
        while True:
            try:
                key, curVal = next(key_iter)
                while rand <= cumulative_prob + curVal:
                    return key
                cumulative_prob += curVal
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            ...
            if noisyDistance == None:
                ...
                totalDistance = manhattanDistance(pacmanPosition, ghostPosition)
                return busters.getObservationProbability(noisyDistance, totalDistance)
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        beliefUpdates = map(lambda ghostPos: self.getObservationProb(observation, pacmanPos, ghostPos, jailPos), self.allPositions)

        for ghostPos, beliefUpdate in zip(self.allPositions, beliefUpdates):
            self.beliefs[ghostPos] *= beliefUpdate
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newDistance = DiscreteDistribution()

        # Initialize iterator for oldPos
        oldPosIterator = iter(self.allPositions)

        # Use a while loop to iterate over oldPos and newPosDist
        while True:
            try:
                oldPos = next(oldPosIterator)
                newPosDist = self.getPositionDistribution(gameState, oldPos)

                # Iterate over newPos and prob using items()
                for newPos, prob in newPosDist.items():
                    newDistance[newPos] = newDistance[newPos] + self.beliefs[oldPos] * prob

            except StopIteration:
                # Break the loop when all oldPos have been processed
                break

        self.beliefs = newDistance
        "*** END YOUR CODE HERE ***"
    def getBeliefDistribution(self):
        return self.beliefs
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        self.particles = [pos for legalPos in [self.legalPositions] * self.numParticles for pos in legalPos]
        "*** END YOUR CODE HERE ***"
    def getBeliefDistribution(self):
        """
        Return the agent's current belief state, a distribution over ghost
        ...
        for particle in self.particles:
            if particle in particle_counts:
                particle_counts[particle] += 1
            else:
                particle_counts[particle] = 1

        total_particles = sum(particle_counts.values())
        distribution = DiscreteDistribution({particle: count / total_particles for particle, count in particle_counts.items()})
        return distribution
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            weight_distribution[particle] += self.getObservationProb(observation, pacman_position, particle, jail_position)

        weight_distribution.normalize()

        # Reinitialize particles if all weights are zero
        while weight_distribution.total() == 0:
            self.initializeUniformly(gameState)
            weight_distribution = self.getBeliefDistribution()

        # Resample particles using a while loop
        resampled_particles = []
        i = 0
        while i < self.numParticles:
            resampled_particles.append(weight_distribution.sample())
            i += 1
    ...
    def elapseTime(self, gameState):
        ...
        # Update new_distribution based on legal positions and their belief values
        current_position_index = 0
        while current_position_index < len(self.legalPositions):
            current_position = self.legalPositions[current_position_index]
            position_distribution = self.getPositionDistribution(gameState, current_position)

            key_index = 0
            while key_index < len(position_distribution.keys()):
                key_value = list(position_distribution.keys())[key_index]
                new_distribution[key_value] += position_distribution[key_value] * belief_distribution[current_position]
                key_index += 1

            current_position_index += 1

        # Resample particles using a loop
        new_particle_list = []
        i = 0
        while i < self.numParticles:
            new_particle_list.append(new_distribution.sample())
            i += 1

        self.particles = new_particle_list
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        # Use a list comprehension to create particleList
        particleList = [pos for _ in range(self.numParticles) for pos in allPositions]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        pacman_position = gameState.getPacmanPosition()
        observation_distribution = DiscreteDistribution()

        # Calculate probabilities for each particle based on observation using a for loop
        for particle in reversed(self.particles):
            probability = 1
            # Calculate the probability for each ghost in a separate for loop
            for ghost_index in reversed(range(self.numGhosts)):
                jail_position = self.getJailPosition(ghost_index)
                probability *= self.getObservationProb(observation[ghost_index], pacman_position, particle[ghost_index], jail_position)
            observation_distribution[particle] += probability

        observation_distribution.normalize()

        # Check if all weights are zero and reinitialize particles
        if observation_distribution.total() == 0:
            self.initializeUniformly(gameState)
            observation_distribution = self.getBeliefDistribution()

        # Resample particles using a list comprehension
        resampled_particles = [observation_distribution.sample() for _ in range(self.numParticles)]
